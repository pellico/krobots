...
fn draw_polyline(gizmos: &mut Gizmos, polyline: &[Vector], scaling_factor: f32) {
    ...
    let poly_vec: Vec<Vec2> = polyline.iter().map(|&x| x * scaling_factor).collect();
    ...
    for tank in physics_state.tanks.values() {
        ...
        if !tank.is_dead() {
            ...
            gizmos
                .arc_2d(
                    Isometry2d::new(
                        tank.position().translation,
                        Rot2::radians(
                            tank.position().rotation.angle() + tank.radar_position()
                                - PI / 2.0
                                - tank.radar_width() * 0.5,
                        ),
                    ),
                    tank.radar_width(),
                    scaled_radar_range,
            ...
            let v2 =
                Vector::from_angle(min_angle).rotate(Vector::new(scaled_radar_range, 0.0)) + v1;
            let v3 =
                Vector::from_angle(max_angle).rotate(Vector::new(scaled_radar_range, 0.0)) + v1;
