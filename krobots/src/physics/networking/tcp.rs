...
impl NetInterface for ClientInterface {
    fn wait_new_tank(&mut self, buffer: &mut [u8]) -> (Box<dyn ClientConnection>, usize) {
        let (mut stream, src) = self
            .listener
            .accept()
            .expect("Error in while waiting for connection for simulator");
        debug!("Connection request form from {}", src);
        //stream.set_nonblocking(false).unwrap();
        stream.set_nodelay(true).unwrap();
        let msg_size = stream
            .read(buffer)
            .expect("Error while receiving tank client registration confirmation");
        ...
        (Box::new(TankClientConnection { stream }), msg_size)
    }
}
